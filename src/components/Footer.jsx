      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          <div className="sm:col-span-2 lg:col-span-1">
            ...
            <p className="text-sm text-muted-foreground leading-relaxed">Northeast Ohio's trusted exterior contractor. Roofing, siding, and gutters — installed with precision, backed by integrity.</p>
            <p className="text-xs text-muted-foreground mt-2">Serving Cuyahoga, Lake, Geauga, Summit, Medina, Portage, Ashtabula & Trumbull Counties.</p>
            <a href="https://www.facebook.com/share/14cDuZ4TLPU/?mibextid=wwXIfr" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 mt-4 text-sm text-primary hover:text-primary/80 transition-colors">
          ...
          <div>
            ...
            <ul className="space-y-2 text-sm text-muted-foreground">
              ...
              <li><a href="/contact" onClick={(e) => { e.preventDefault(); openAssistant(); }} className="hover:text-primary transition-colors">Ask Demore / Free Estimate</a></li>
              <li><a href="https://media.base44.com/files/public/user_6a22dc88783b484dd6ef2b08/6a43445b3_DemoreContingencyAgreement.pdf" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition-colors">Contingency Agreement</a></li>
            </ul>
          </div>
        ...
        {isLoggedIn && (
          <div className="border-t border-border/50 pt-6 mb-2 flex flex-wrap items-center gap-x-6 gap-y-2">
            <span className="inline-flex items-center gap-1.5 text-xs uppercase tracking-wider text-muted-foreground"><Shield className="w-3.5 h-3.5 text-primary" /> Staff</span>
            <a href="/ops" className="text-xs text-muted-foreground hover:text-primary transition-colors">Ops Dashboard</a>
            ...
            <button onClick={() => base44.auth.logout()} className="inline-flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary transition-colors"><LogOut className="w-3.5 h-3.5" /> Log Out</button>
          </div>
        )}

        <div className="border-t border-border/50 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-muted-foreground">© {new Date().getFullYear()} Demore Exterior Solutions. All rights reserved.</p>
          <p className="text-xs text-muted-foreground">Licensed & Insured General Contractor · Mentor, Ohio</p>
        </div>
      </div>
    </footer>
  );
