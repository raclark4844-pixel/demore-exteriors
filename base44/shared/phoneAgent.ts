...
export const BUSINESS_DAYS = [1, 2, 3, 4, 5, 6]; // Monday–Saturday
...
export function isStormCall(input: Record<string, unknown>): boolean {
  ...
  if (text(input.claim_carrier, 80) || text(input.claim_number, 80) || text(input.adjuster_name, 80) || text(input.date_of_loss, 80)) {
    return true;
  }
  ...
  const intent = text(input.service_needed || input.service_type, 120).toLowerCase();
  return STORM_HINTS.some((hint) => intent.includes(hint));
}
